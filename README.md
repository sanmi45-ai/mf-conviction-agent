...
Data source: [mfdata.in](https://mfdata.in) — a free, no-auth API that
...
on:
  schedule:
    # Try daily from the 12th to the 20th. mfdata.in is a small free API
    # that occasionally has downtime, so instead of one shot on a fixed
    # day, we retry daily until it succeeds. The script skips itself once
    # this month's file already exists, so extra days are harmless no-ops.
    - cron: '0 9 12-20 * *'   # 9am UTC, days 12–20 of every month
...
**Why daily instead of once a month:** mfdata.in is a small, free,
community-run API — it can have occasional downtime (you may see this as a
"522" error in the Action's log). Running once on a fixed day risks the
whole month's data collection failing on a bad day. Running daily during a
9-day window means it just keeps quietly retrying until the site's back up,
then produces the file and skips itself for the rest of the month. If it's
*still* failing after several days, that's worth investigating rather than
treating it as a normal blip.
